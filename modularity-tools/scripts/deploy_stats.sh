...
#./example.sh GF_HOME ID(REVISION) TARGET_DIRECTORY STATS_XML_FILE_NAME TIME_XML_FILE_NAME MEMORY_XML_FILE_NAME LOG_FILE_NAME APP_PATH
...
echo -e "\nDEPLOYING $DEPLOY_FILE" >> $REPO_DIR/$MODULES_OUTPUT_FILE

for i in `seq 1 $ITER`
do
rm -rf $LOG_DIR/server.log
rm -rf $GF_HOME/domains/domain1
$GF_HOME/bin/asadmin create-domain --nopassword=true domain1 
banner="$banner TRY$i$TAB"
$GF_HOME/bin/asadmin start-domain
sleep $SLEEP_SECS2

$GF_HOME/bin/asadmin list-modules | grep 'OSGiModuleImpl:: Bundle' | sed "s/^.*Bundle = //" > $REPO_DIR/results_before.txt
...
/usr/bin/time -o $TIME_FILE --format "%E" $GF_HOME/bin/asadmin deploy $DEPLOY_FILE
...
$GF_HOME/bin/asadmin list-modules | grep 'OSGiModuleImpl:: Bundle' | sed "s/^.*Bundle = //" > $REPO_DIR/results_after.txt
cp $REPO_DIR/results_after.txt $REPO_DIR/results_command.txt
...
UNDEPLOY_FILE=`echo $DEPLOY_FILE | sed "s/^.*\///" | sed 's/....$//'`
$GF_HOME/bin/asadmin undeploy $UNDEPLOY_FILE
...
FILE="$REPO_DIR/module_states_$UNDEPLOY_FILE.xml"
...
IFS=";"
for word in $MODULE_STATES; do
cat $REPO_DIR/results_command.txt | grep $word | sed "s/^.*Bundle = \[//" | sed "s/ \[.*$//"> $REPO_DIR/temp.txt
while read line      
do
sed -ie '$d' $FILE
prepare="<Module>\n<state>$word</state>\n<name>$line</name>\n</Module>\n</Modules>"
