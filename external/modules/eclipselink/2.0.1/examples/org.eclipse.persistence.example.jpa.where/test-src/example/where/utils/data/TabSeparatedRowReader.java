...
public class TabSeparatedRowReader {
	...
	public TabSeparatedRowReader(String fileName) {
		this.dataFile = GeonamesDataFiles.getLocalFile(fileName, true);
		verifyDataFile();
	}
	...
	public boolean ready() {
		try {
			...
			throw new RuntimeException("TabSeparatedRowReader.ready fialed: " + e.getMessage(), e);
		}
	}
	...
	public void skipLinesUntil(String lineStart) {
		String line = "";

		try {
			while (getReader().ready() && !line.startsWith(lineStart)) {
	...
	public List<String> readLine() {
		...
		try {
			...
			throw new RuntimeException("TabSeparatedRowReader.readLine failed: " + ioe.getMessage(), ioe);
		}
