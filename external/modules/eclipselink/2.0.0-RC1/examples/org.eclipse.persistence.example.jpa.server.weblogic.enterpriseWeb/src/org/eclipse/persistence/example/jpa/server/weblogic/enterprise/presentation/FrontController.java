...
package org.eclipse.persistence.example.jpa.server.weblogic.enterprise.presentation;
 ...
 *   
 * Example URLs
 * http://127.0.0.1:7001/example/FrontController?action=demo
 */
public class FrontController extends HttpServlet implements Servlet {
   ...
   private DataSource datasource = null;
    ...
    /**
     * Test an application managed persistence unit
     */
    public void applicationManagedPersistence() {
        EntityManagerFactory anEMF  = Persistence.createEntityManagerFactory("exampleLocal");
        EntityManager anEM = anEMF.createEntityManager();
        Query aQuery = null;
        List<Cell> rowsList = null;
        try {
            aQuery = anEM.createQuery(exampleJPQL);
            rowsList = aQuery.getResultList();
            if(null != rowsList) {
                Iterator<Cell> anIterator = rowsList.iterator();
                System.out.println(rowsList.size() + " Entities in storage: " + rowsList.size());
                
                // Generate an HTML table summary of all entities in the database
                int objectCounter = 0;
                while(anIterator.hasNext()) {
                    Cell anObject = anIterator.next();
                    System.out.println("[EL ExampleLocal EM]: enterprise: Object: " + anObject);
                    objectCounter++;
                }
            } else {
                System.out.println("No rows returned or Invalid JPQL <br/>");
            }               
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void configureGlobalWebLogicDataSource(PrintWriter out, boolean x) {
        Context aContext = null;  
        Hashtable<String, String> aHashTable = new Hashtable<String, String>();  
        aHashTable.put(Context.INITIAL_CONTEXT_FACTORY, "weblogic.jndi.WLInitialContextFactory");  
        aHashTable.put(Context.PROVIDER_URL,  "t3://127.0.0.1:7001");  
        //Connection aConnection = null;  
        try {    
            aContext = new InitialContext(aHashTable);   
            // Must match the datasource in persistence.xml
            datasource = (javax.sql.DataSource) aContext.lookup ("localJTA");
            if(null == datasource) {
                System.out.println(">> Global DataSource is null");
                out.println("<br/>Global DataSource is null");                
            } else {
                Connection aConnection = datasource.getConnection();
                System.out.println(">> Global DataSource Connection: " + datasource.toString());
                out.println("<br/>Global DataSource Connection: " + datasource.toString());
                if(null != aConnection) {
                    aConnection.close();
                }
            }
    ...
    private void processCreateDatabaseCommand(HttpServletRequest request, HttpServletResponse response,PrintWriter out) {
        // Insert schema and classes into the database
        ...
        try {
            ...
            System.out.println("[EL Example]: enterprise: _createDatabase() Inserted: " + cell1);            
            getEntityManager().persist(cell2);
            System.out.println("[EL Example]: enterprise: _createDatabase() Inserted: " + cell2);            
            // Store objects        
    ...
    private void processGliderCommand(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        generateGlider(out);
        //applicationManagedPersistence();
        // Get a class from the db
        List<Cell> rowsList = (List<Cell>)getApplicationService().query("select object(e) from Cell e");
        if(null != rowsList) {
            Iterator<Cell> anIterator = rowsList.iterator();
