...
public class CreateSchema {
	...
	public static void main(String[] args) {
		...
		emf = PersistenceHelper.createEMF(properties, true);
	...
	private static void verifyDatabaseEmpty(EntityManager em) {
		...
		for (Iterator i = server.getDescriptors().values().iterator(); i
				.hasNext();) {
			RelationalDescriptor descriptor = (RelationalDescriptor) i.next();

			if (!descriptor.isAggregateDescriptor()) {
				ReportQuery rq = new ReportQuery(descriptor.getJavaClass(),
				...
				System.out.println("COUNT(" + descriptor.getAlias() + "): "
						+ count);
			}
		}
	}
