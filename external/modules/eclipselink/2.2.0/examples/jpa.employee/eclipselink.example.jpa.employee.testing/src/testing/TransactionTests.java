...
public class TransactionTests extends EclipseLinkJPATest {
    ...
    public void loadGroupSerializeMerge() {
        ...
        // clear shared cache to make the number of selects predictable:
        // if the Employee object is already in the shared cache without a fetch group,
        // then one less select would be executed - no need to read the whole object on merge.
        ((JpaCache)em.getEntityManagerFactory().getCache()).clear();
        ...
        assertEquals(123456.0, emp.getSalary());
    ...
    public void copyGroupMerge() {
        ...
        assertEquals(3, getQuerySQLTracker(em).getTotalSQLSELECTCalls());
        assertEquals(3, getQuerySQLTracker(em).getTotalSQLUPDATECalls());
        assertEquals(1, getQuerySQLTracker(em).getTotalSQLINSERTCalls());
