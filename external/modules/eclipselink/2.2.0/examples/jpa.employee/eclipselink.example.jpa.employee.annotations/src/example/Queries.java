...
public class Queries {
    ...
    public List<Employee> joinFetchAndBatchHints(EntityManager em) {
        TypedQuery query = em.createQuery("SELECT e FROM Employee e WHERE e.manager IS NOT NULL and SIZE(e.managedEmployees) > 0", Employee.class);
    ...
    /**
     * Example of using EclipseLink's casting in criteria queries using AS.
     * This query will find all employees who have a LargeProject with a budget
     * greater then 1000 and at least one SmallProject with a name ending in
     * 'Reporter'.
     * 
    ...
    public List<Employee> findUsingTreatAsCriteria(EntityManager em) {
        ...
        Path largeProjectBudget = ((Path) empRoot.join("projects").as(LargeProject.class)).get("budget");
        Path smallProjectName = ((Path) empRoot.join("projects").as(SmallProject.class)).get("name");

        cq.where(qb.and(qb.greaterThan(largeProjectBudget, 1000), qb.like(smallProjectName, "%Reporter")));
